...
def drawBoard(cards):
  # Takes an array of dictionaries, each with keys: card, number, pattern, shape
  boardSVG = prefix + body
  ...
  for i in range(4):
    for j in range(3):
      ...
      boardSVG += mkCard(x, y)
      card = cards[(i * 3) + j]
      boardSVG += drawCardContents(x, y, card["number"], card["color"], card["shape"], card["pattern"])
